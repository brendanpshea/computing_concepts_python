...
1. Follow the factorial procedure by hand for $n = 6$. Then change one line so it computes $1 + 2 + 3 + \ldots + n$ instead. What is the new line? What is the new output for $n = 6$?
2. The interpreter only knows `+`, `*`, and numbers. Extend the pseudocode so it also handles `-` (subtract). Write one short program in the new instruction set. What is its output?
