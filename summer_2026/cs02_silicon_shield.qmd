...
Gordon Moore observed in 1965 that the number of transistors on a chip was doubling about every two years. This is exponential growth. Exponential growth destroys our intuitions. If you double something every two years for fifty years, you do not get fifty doublings. You get $2^{25}$ doublings — about 33 million times the original. The Intel 4004 microprocessor of 1971 had 2,300 transistors. A modern Apple M2 Ultra has about 134 billion. The arithmetic is small enough to run in a few lines:

```python
# Moore's-Law: double transistors every 2 years from the Intel 4004 (1971).
start_year, start_count = 1971, 2_300
target = 100_000_000_000     # 100 billion transistors

year, count = start_year, start_count
while count < target:
    year += 2
    count *= 2
print(f"At Moore's rate, 100 B transistors arrives in {year}.")
print(f"Predicted count that year: {count:,}.")

# Reality has bent the curve. Compare to actual milestones:
real = {1971: 2_300, 1985: 275_000, 1999: 9_500_000,
        2010: 1_170_000_000, 2024: 134_000_000_000}
for y, c in sorted(real.items()):
    print(f"{y}: {c:>15,} transistors")
```

Run it. The pure Moore prediction reaches 100 billion transistors in the late 2020s. The actual milestones show how close — and where — the curve bent in the 2010s. Moore's Law has been slowing for about a decade. Each additional doubling now costs more dollars and more physics. This is part of why the leading-edge fab is so concentrated. Only a handful of companies can afford to chase the next node. As of 2025, only one — TSMC — is making the very latest chips in volume.

The second fact is what those transistors *let you represent*. A chip stores and moves data in bits. The [bit width]{.key-term} of its memory addresses sets a hard ceiling on what software can do. A few lines make the ceiling visible:

```python
# What can you reach with N-bit memory addresses (one byte per address)?
def reach(n_bits):
    n_addresses = 2 ** n_bits
    for unit, size in [("EB", 2**60), ("TB", 2**40),
                       ("GB", 2**30), ("MB", 2**20), ("KB", 2**10)]:
        if n_addresses >= size:
            return f"{n_addresses // size} {unit}"
    return f"{n_addresses} bytes"

for n in [8, 16, 32, 64]:
    print(f"{n}-bit addresses can reach {reach(n)}")
```

The output is a small lesson in scale:

- An 8-bit machine can address 256 bytes.
- A 16-bit machine can address 64 KB.
- A 32-bit machine can address 4 GB.
- A 64-bit machine can address 16 EB — 16 billion gigabytes.
...
1. Run the Moore's-Law snippet. Change the doubling period from 2 years to 3 years. By what year does the prediction reach 100 billion transistors? What does this tell you about exponential growth?
2. Modify the `reach` function to count *FP32 floats* (4 bytes each) instead of single bytes. About how many FP32 parameters fit in a 16 GB GPU? How many FP16? How many FP8?
...
4. You are at the U.S. Bureau of Industry and Security in March 2026. Your draft rule restricts a new chip. Huawei is expected to respond with its own version within a year. Do you publish the rule? Why?
