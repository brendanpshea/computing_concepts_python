...
The first is what Lovelace meant by an *algorithm*. An [algorithm]{.key-term} is a finite list of unambiguous steps that, followed exactly, produces a specific result. Lovelace's Note G gave one for the Bernoulli numbers — a sequence of fractions $B_0, B_1, B_2, \ldots$ that appear all over mathematics. Her full procedure, written as a table of operations for the Engine, runs several pages. The modern shape of the same idea is short enough to run on a phone:

```python
from fractions import Fraction

def bernoulli(n):
    # Akiyama-Tanigawa: a small loop that, after n+1 passes,
    # leaves the n-th Bernoulli number in the first slot.
    a = [Fraction(0)] * (n + 1)
    for m in range(n + 1):
        a[m] = Fraction(1, m + 1)
...
for n in range(8):
    print(f"B{n} = {bernoulli(n)}")
```

Run it and you get `B0 = 1, B1 = 1/2, B2 = 1/6, B3 = 0, B4 = -1/30, B5 = 0, …`. The program has no idea what Bernoulli numbers are. It just follows the rules. Following the rules is, in Lovelace's terms, the Engine's province. Designing the rules — *which* arithmetic, in *which* order, to make the right pattern fall out — was hers.
...
def interpret(program):
    # `program` is a list of strings — instructions written as data.
    # The interpreter has no idea what computation it is doing; it
    # just runs whatever it is handed.
    stack = []
    for op in program:
        if   op == "+": stack.append(stack.pop() + stack.pop())
        elif op == "*": stack.append(stack.pop() * stack.pop())
        else:           stack.append(int(op))   # treat as a number
    return stack[-1]

print(interpret(["3", "4", "+", "5", "*"]))   # (3 + 4) * 5  = 35
print(interpret(["6", "7", "*"]))             # 6 * 7        = 42
```

This ten-line interpreter is a working — if tiny — universal machine. Hand it one list of instructions and it does arithmetic; hand it a different list and it does different arithmetic; in principle, hand it the right (much larger) list and it could simulate any other computer, including itself. That is what Turing's 1936 paper proved formally: one machine, parameterised by its program, can stand in for all of them.
...
1. Run the `bernoulli` snippet. Print the first 12 Bernoulli numbers instead of the first 8. What do you notice about $B_3, B_5, B_7, B_9, B_{11}$? Why might that pattern have interested Lovelace?
2. The tiny interpreter only knows `+`, `*`, and numbers. Add a `-` (subtract) operation. Write one new program for it. Predict the output before you run it.
